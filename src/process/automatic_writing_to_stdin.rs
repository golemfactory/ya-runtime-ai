use std::{path::PathBuf, process::{ExitStatus, Stdio}, sync::Arc, time::Duration};
...
impl Runtime for Automatic {
    async fn start(model: Option<PathBuf>) -> anyhow::Result<Automatic> {
        ...
        let stdout = child
            .stdout
            .take()
            .context("Failed to get Automatic stdout")?;
        let stderr = child
            .stderr
            .take()
            .context("Failed to get Automatic stderr")?;
        let mut stdin = child
            .stdin
            .take()
            .context("Failed to get Automatic stdin")?;
        ...
        loop {
            match tokio::select! {
                ...
                Some(Err(err)) => {
                    log::error!("! Failed to read line. Err {err}");
                    break;
                },
                None => {
                    log::error!("! Nothing");
